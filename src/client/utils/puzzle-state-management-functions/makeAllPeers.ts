...
/** makeAllPeers
 *
 * Two squares are peers if they can't simultaneously hold the same number in a Sudoku puzzle. This
 * includes any two numbers in the same row, column, or box (set of 9 squares) are peers.
 * makeAllPeers generates an allPeers object that holds key:value pairs where the key is a SquareId
 * string (e.g. 'A1') and the value is a set of all of the squareId string peers
 * (e.g. 'A2', 'A3',...) of that key.
 * This set will be used in the styling of the squares. Other groupings of squares is also returned
 * for use in the Sudoku grid.
 *
 * @returns an allPeers object, a boxes array that holds 9 sets of the squares that form the
 * sudoku grid, a rows array that holds 9 sets of squareIds in each row, and a column array that
